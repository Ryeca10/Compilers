...
let mult: int -> int -> int = fun (x:int) -> fun (y:int) -> x * y
...
let cents_of : int -> int -> int -> int -> int = 
      fun (q:int) -> fun (d:int) -> fun (n:int) -> fun (p:int) -> 
          25 * q + 10 * d + 5 * n + p
...
*)
let prob3_case3 : int =
  let aux = prob3_case2 10 in double aux
...
let rev_t (l: 'a list) : 'a list =
    let rec helper (l1: 'a list) (l2:'a list) : 'a list =
        begin 
          match l1 with
            | [] -> l2
            | h :: t -> helper t (h::l2)
   ...
   | h1::t1 -> 
            begin
               ...
               | h2::t2 -> 
                        begin
                          match h1 < h2 with
                          | true -> h1::h2::union t1 t2
                          | false -> 
                                    begin
                                      match h1 = h2 with
                                      | true -> h1::union t1 t2
                                      | false -> h2::h1::union t1 t2
                                    end
                        end
             end
 end
...
(*
  ...
  Each expression denotes an int64 value, 
  but since it can contain variables,
    ...
    | Add(x,y) -> 
            begin 
              ...
              | Const x0 ->
                            begin
                              match y with
                              | Const y0 -> Const (Int64.add x0 y0)
                              | Var y0 -> Add (optimize x, y)
                              | _ -> optimize (Add (x, optimize y))
                            end
              | Var x0 -> Add (x, optimize y)
              | _ -> 
                  begin 
                    match y with
                    | Const 0L -> optimize x
                    | Const y0 -> optimize (Add (optimize x, y))
                    | Var y0 -> Add (optimize x, y)
                    | _ -> optimize (Add (optimize x, optimize y))
                  end
            end
    | Mult(x,y) ->  
           begin 
              ...
              | Const x0 ->
                    begin
                      match y with
                            | Const y0 -> Const (Int64.mul x0 y0)
                            | Var y0 -> Mult (optimize x, y)
                            | _ -> optimize (Mult (x, optimize y))
                    end
              | Var x0 -> Mult (x, optimize y)
              | _ -> 
                  begin 
                    match y with
                    | Const 0L -> Const 0L
                    | Const 1L -> optimize x
                    | Const y0 -> optimize (Mult (optimize x, y))
                    | Var y0 -> Mult (optimize x, y)
                    | _ -> optimize (Mult (optimize x, optimize y))
                  end
            end
    | Neg x -> 
              match x with
              | Const x0 -> Const (Int64.neg x0)
              | Var x0 -> Neg x
              | _ -> optimize (Neg (optimize x))
  end
...
let rec compile (e:exp) : program =
  begin
    ...
    | Var x -> [IPushV x]
    | Const x -> [IPushC x]
    | Add(x,y) -> append (append (compile x) (compile y)) [IAdd]
    | Mult(x,y) -> append (append (compile x) (compile y)) [IMul] 
